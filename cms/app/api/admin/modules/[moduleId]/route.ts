...
async function proxy(method: 'GET' | 'PATCH' | 'DELETE', req: NextRequest, ctx: Ctx) {
  ...
  const res = await fetch(`${apiBaseUrl}/v1/admin/modules/${moduleId}`, { method, cache: 'no-store', headers: { Authorization: `Bearer ${getAdminToken(req)}`, ...(body ? { 'Content-Type': 'application/json' } : {}) }, body });
