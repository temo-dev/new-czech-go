...
async function proxy(method: 'GET' | 'POST', request: NextRequest) {
  ...
  const url = `${apiBaseUrl}/v1/admin/modules${qs ? '?' + qs : ''}`;
  const res = await fetch(url, { method, cache: 'no-store', headers: { Authorization: `Bearer ${adminToken}`, ...(body ? { 'Content-Type': 'application/json' } : {}) }, body });
