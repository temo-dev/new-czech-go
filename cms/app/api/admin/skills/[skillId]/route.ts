...
async function proxy(method: 'GET' | 'PATCH' | 'DELETE', req: NextRequest, ctx: Ctx) {
  ...
  const res = await fetch(`${apiBaseUrl}/v1/admin/skills/${skillId}`, { method, cache: 'no-store', headers: { Authorization: `Bearer ${adminToken}`, ...(body ? { 'Content-Type': 'application/json' } : {}) }, body });
