...
## Post-Implementation Addendum (2026-05-04)

Manual logs sau Phase 5 cho thấy ElevenLabs trả audio nhanh (`first_agent_audio` ~1–4s) nhưng Simli avatar có thể `SPEAK` sau 11–15s. Vì vậy V16 được mở rộng bằng các post-smoke fixes:

- Simli avatar là learner opt-in trong Profile; sound-wave local audio là mặc định.
- Profile có `Âm lượng giám khảo` 100–180% cho sound-wave mode.
- Sound-wave playback chuyển audio session sang `playback/spokenAudio`; sound-wave PTT mic dùng `playAndRecord + measurement`; Simli duplex dùng `playAndRecord + videoChat`.
- Sound-wave PTT giữ `record` không tự đổi iOS audio session và boost outbound PCM16 `2.4x` có clipping để ElevenLabs VAD bắt giọng nhỏ tốt hơn; log `rawPeak`/`sentPeak` + `vadMax`.
- Local playback phải phát xong mới enable mic; `PcmAudioPlayer.flushAndPlay()` serialize drain bằng `_flushFuture`.
- Local chunk flush không tự mở mic; chỉ `agent_response_complete` hoặc silence timeout mới complete turn; flush defer khi mic active/transition để tránh iOS `!pri`.
- Session UI có scroll lane riêng cho transcript/prompt và fixed controls lane; widget test 360×640 bắt overflow compact screen.
- `_agentWaitTimer` tăng 8s → 12s để giảm false timeout khi ElevenLabs phản hồi chậm.
