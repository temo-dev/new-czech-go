...
/// Simulator question screen — premium, full exam simulation.
///
/// extras: `{ 'exerciseId': String, 'totalCount': int, 'attemptId': String? }`
class SimulatorQuestionScreen extends ConsumerStatefulWidget {
  const SimulatorQuestionScreen({super.key, required this.index});

  final int index;

  @override
  ConsumerState<SimulatorQuestionScreen> createState() =>
      _SimulatorQuestionScreenState();
}

class _SimulatorQuestionScreenState
    extends ConsumerState<SimulatorQuestionScreen> {
  ...
  String? get _exerciseId => _extra?['exerciseId'] as String?;
  int get _totalCount => _extra?['totalCount'] as int? ?? 1;
  ...
  Widget build(BuildContext context) {
    ...
    if (exerciseId == null) {
      return Scaffold(
        ...
        body: ErrorState(
          message: 'Không tìm thấy câu hỏi. Vui lòng quay lại.',
          onRetry: () => context.go(AppRoutes.simulatorIntro),
        ),
      );
    }
    ...
    return Scaffold(
      backgroundColor: AppColors.surface,
      appBar: _buildAppBar(context, widget.index, _totalCount),
      body: exerciseAsync.when(
        ...
        data: (question) => _QuestionBody(
          ...
          onNext: _isLastQuestion
              ? () => context.pushReplacement(AppRoutes.simulatorResult)
              : () => context.push(
                    AppRoutes.simulatorQuestionPath(widget.index + 1),
                    extra: _extra,
                  ),
  ...
  bool get _isLastQuestion => widget.index >= _totalCount - 1;
  ...
  void _showExitDialog(BuildContext context) {
    showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Thoát mô phỏng?'),
        content: const Text(
            'Tiến trình hiện tại sẽ không được lưu nếu bạn thoát.'),
        actions: [
          TextButton(
          ...
          TextButton(
            onPressed: () {
              ...
              context.go(AppRoutes.simulatorIntro);
            },
            child: Text(
              'Thoát',
...
class _CtaButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: canTap ? (isSubmitted ? onNext : onSubmit) : null,
      child: Container(
        height: 56,
        decoration: BoxDecoration(
