...
// ── Replicate client ──────────────────────────────────────────────────────────
//
// Model ID is centralized in processing.LoadLLMModels().ReplicateImage
// (override via REPLICATE_IMAGE_MODEL env). Default: flux-schnell.
...
func createReplicatePrediction(ctx context.Context, apiKey, prompt string) (string, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost,
		replicateBaseURL+"/v1/models/"+processing.LoadLLMModels().ReplicateImage+"/predictions",
