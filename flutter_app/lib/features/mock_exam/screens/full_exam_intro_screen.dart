...
class _FullExamIntroScreenState extends State<FullExamIntroScreen> {
  ...
  bool get _allDone =>
      widget.test.sections.every((s) => _attemptIds.containsKey(s.sequenceNo));
  ...
  Future<void> _openSection(MockTestSection section) async {
    ...
    if (detail.isCteni) {
      ...
      if (!mounted) return;
      setState(() => _error = AppLocalizations.of(context).exerciseOpenError);
      return;
    }
    if (mounted && attemptId != null) {
      setState(() {
        _attemptIds[section.sequenceNo] = attemptId!;
        _error = null;
      });
    }
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      final attemptIds =
          widget.test.sections.map((s) => _attemptIds[s.sequenceNo]!).toList();
      ...
      await Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder:
              (_) => FullExamResultScreen(session: session, test: widget.test),
        ),
      );
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text(
          AppLocalizations.of(context).fullExamScreenTitle,
        ...
        child: ListView(
          ...
          children: [
            ...
            Row(
              children: [
                _StatChip(
                  label: AppLocalizations.of(context).fullExamDurationLabel,
                  value: AppLocalizations.of(
                    context,
                  ).fullExamMinDuration(test.estimatedDurationMinutes),
                ),
                const SizedBox(width: AppSpacing.x3),
                _StatChip(
                  label: AppLocalizations.of(context).fullExamMaxPtsLabel,
                  value: AppLocalizations.of(context).fullExamPts(maxPts),
                ),
                const SizedBox(width: AppSpacing.x3),
                _StatChip(
                  label: AppLocalizations.of(context).fullExamPassLabel,
                  value: AppLocalizations.of(
                    context,
                  ).fullExamPassSymbol(passThreshold),
                ),
              ],
            ),
            ...
            // Sections
            Text(
              AppLocalizations.of(context).fullExamSectionsTitle,
              ...
              return Padding(
                ...
                child: ListTile(
                  ...
                  shape: RoundedRectangleBorder(
                    ...
                    side: BorderSide(
                      color:
                          done ? AppColors.success : AppColors.outlineVariant,
                    ),
                  ),
                  leading: Icon(
                    done
                  ...
                  title: Text(
                    sec.exerciseType.replaceAll('_', ' ').toUpperCase(),
            ...
            FilledButton(
              ...
              child:
                  _submitting
                      ...
                      : Text(
                        _allDone
                            ? AppLocalizations.of(context).fullExamSubmitCta
                            : AppLocalizations.of(context).fullExamSubmitHint,
